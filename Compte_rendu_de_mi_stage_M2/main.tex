...
Au cours de la première partie de ce stage, deux grands objectifs ont été fixés. Le premier était d'évaluer, de comparer les méthodes de réduction de dimension déjà existantes et, si possible, de proposer une méthode qui semble la meilleure puis le deuxième qui était d'étudier les corrélations entre les différents angles de la molécule. \Javier{Explain what is the interest of the second part: in order to define better dimensionality reduction techniques, we need to understand the dependencies between the angular variables, so... }

\Javier{I think the structure of the following sections is not very clear. I would write to sections (one for each of the objectives you mentioned above). Then, you can start with a subsection `Recherche bibliographique' inside each of the sections, then another call e.g. `methods` where you explain what tools/analyses did you use, and then `results` where you summarize the results (each big section has the same structure which is clearer). Also no need to separate the benchmark and the newer data sets into two different subsections, you can just explain the results once and say that the same results were found in two different data sets (no need to give more details here).}

\section{Recherches bibliographiques}

\Javier{I modifed the citation format to numbers: remove the parentheses and use only \cite{sargsyanGeoPCANewTool2012}.}
...
La deuxième partie est l'étude d'une éventuelle dépendance entre les angles. Pour cela, il a été choisi d'utiliser dans un premier temps les tests issus du transport optimal et en particulier avec la distance de Wasserstein entre les distributions des angles (\cite{peyreComputationalOptimalTransport2020}) utilisée dans \deleted{le papier de} \cite{gonzalez-delgadoTwosampleGoodnessoffitTests2023}. \Javier{The following sentence is not very clear, try to write it more formally. Also use $\mathbb{P}$ for a probability, or you can use $\mathcal{L}(X)$ to denote the probability law of $X$ (saying so).} Plus précisement, il a été étudié les lois $P(X|Y=y)$ et $P(X)$ pour $X$ et $Y$ des variables aléatoires dans $\mathbb{S}^1$. L'objectif est de calculer la distance \added{de Wasserstein} entre ces objets \deleted{avec le transport optimal} pour trancher si \deleted{oui ou non} une égalité des lois est imaginable. Les statistiques et p-values de ces tests ont été obtenus pour $\{y_k \in [5k, 5(k+1)] | k = 0,..., 71\}$, \Javier{Replace the previous sentence saying only that you calculated the distance across a grid of values of $Y$, then computed the expected distance.} pour chaque angle, et chaque base. Cette analyse a un coût élevé en temps de calcul ce qui fait que des méthodes comme des tests par permutations ne sont pas envisageables. Etant donné le nombre de tests effectué, une correction de Bonferroni-Holm était indispensable sur les résultats. La significativité des résultats est encore en cours d'étude. \Javier{Maybe you can say that you have already computed all the $p$-values.}
...
\Javier{Mention what are the next steps in the last two months: using the detected dependency structures to try to define new pseudo-angles that better account for the local RNA structure, trying to solve the limitations described in step 1.}
