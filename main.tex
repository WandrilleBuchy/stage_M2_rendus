...
\begin{frame}
  ...
  \begin{columns}[b] 
    ...
    \begin{column}{0.5\textwidth}
      \raggedleft 
      \raisebox{0.5cm}{\includegraphics[height=1.3cm]{logo_LAAS.png}}
    \end{column}
  \end{columns}
...
\begin{frame}{Goals}
    ...
    \begin{block}{Goal}
        ($\alpha_1$, ... , $\alpha_K$) $\in \mathbb{T}^K$, $7 \leq K \leq 12$ $\rightarrow$ \text{Two-dimensional Space}
    \end{block}
...
\begin{frame}{Goal}
    ...
    \begin{block}{Remark}
        The classical test methods are not usable for $\mathbb{S}^1$ so we used an Optimal Transport based method.
    \end{block}
    }
